...
(function(){
  ...
  const init=()=>{W=c.width=innerWidth;H=c.height=innerHeight;S=[];
    for(let i=0;i<200;i++)S.push({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1.1+.15,
      vx:(Math.random()-.5)*.016,vy:(Math.random()-.5)*.016,
      ph:Math.random()*Math.PI*2,sp:Math.random()*.002+.0005,
      t:Math.random()<.06?'c':Math.random()<.04?'p':'w'});};
  let f=0;
  const draw=()=>{f++;x.clearRect(0,0,W,H);S.forEach(s=>{
    s.x+=s.vx;s.y+=s.vy;
    if(s.x<0)s.x=W;if(s.x>W)s.x=0;if(s.y<0)s.y=H;if(s.y>H)s.y=0;
    const b=.2+.45*Math.abs(Math.sin(f*s.sp+s.ph));
    x.beginPath();x.arc(s.x,s.y,s.r,0,Math.PI*2);
    if(s.t==='c'){x.fillStyle=`rgba(200,155,60,${b*.5})`;if(b>.65){x.shadowColor='#C89B3C';x.shadowBlur=4;}}
    else if(s.t==='p'){x.fillStyle=`rgba(26,58,84,${b*.6})`;if(b>.65){x.shadowColor='#1A3A54';x.shadowBlur=3;}}
...
function _show(p){
  ...
  if(p==='home'){const h=document.getElementById('nl-home');if(h)h.classList.add('on');}
  if(p==='contact'){const cn=document.getElementById('nl-con');if(cn)cn.classList.add('on');}
  window.scrollTo({top:0,behavior:'smooth'});
  setTimeout(rvAll,80);
  setTimeout(animBars,300);
  // Arrows
  const arr=document.getElementById('navArrows');
  if(arr) arr.style.display=HIST.length>1?'flex':'none';
  const bb=document.getElementById('backBtn');
  if(bb) bb.disabled=HPOS<=0;
  const fb=document.getElementById('fwdBtn');
  if(fb) fb.disabled=HPOS>=HIST.length-1;
...
/* ═══ REVEAL ═══ */
function rvAll(){
  const o=new IntersectionObserver(e=>{e.forEach(el=>{if(el.isIntersecting)el.target.classList.add('vis');});},{threshold:.01,rootMargin:'0px 0px -20px 0px'});
  document.querySelectorAll('.rv:not(.vis)').forEach(el=>o.observe(el));
  // Force-reveal all elements after 2.5s as safety net
  setTimeout(()=>{document.querySelectorAll('.rv').forEach(el=>el.classList.add('vis'));},2500);
}
rvAll();
...
var _applyModalEl = document.getElementById('applyModal');
if(_applyModalEl){ _applyModalEl.addEventListener('click',function(e){if(e.target===this)closeApply();}); }
...
/* INIT */
function _initHome(){ gP('home'); }
if(document.readyState==='complete'||document.readyState==='interactive'){ _initHome(); }
else { document.addEventListener('DOMContentLoaded', _initHome); }
...
(function(){
  ...
  var SITES = [];
  SITES.push({url:'mmagha-final-website.vercel.app', live:'https://mmagha-final-website.vercel.app/', iframe:true, tag:'Automotive &middot; Vereeniging', name:'MMAGHA Auto Mags, Rims &amp; Tyres', desc:'Full service site for a Vaal Triangle wheel &amp; tyre workshop &mdash; service catalogue, gallery, blog, and WhatsApp-first contact flow.'});
  SITES.push({url:'fmdelectrical.co.za', live:'https://fmdelectrical.co.za/', iframe:false, icon:'&#9889;', label:'FMD Electrical', tag:'Electrical &middot; Solar &middot; Security', name:'FMD Electrical (Pty) Ltd', desc:'Custom domain build for a Vereeniging electrical, solar, and security specialist &mdash; credentials, services, and direct-call CTAs front and center.'});
  SITES.push({url:'mammies-beauty-palace.vercel.app', live:'https://mammies-beauty-palace.vercel.app/', iframe:true, tag:'Beauty &amp; Wellness', name:'Mammie&#39;s Beauty Palace', desc:'Luxury-positioned beauty salon site covering hair, nails, facials, brows &amp; lashes, and body treatments &mdash; built around WhatsApp bookings.'});
  SITES.push({url:'dynamicspecs-files.vercel.app', live:'https://dynamicspecs-files.vercel.app/', iframe:true, tag:'Healthcare &middot; Optometry', name:'Dynamicspecs Optometrists', desc:'Professional practice site for an optometrist in Vanderbijlpark CBD &mdash; clear service info and easy ways for patients to get in touch.'});
  SITES.push({url:'bob-hardware.vercel.app', live:'https://bob-hardware.vercel.app/', iframe:true, tag:'Hardware &amp; Building Materials', name:'Bob&#39;s Hardware', desc:'Full e-commerce-style catalogue site for a Vaal Region hardware store &mdash; product categories, offers, and a built-in WhatsApp chat assistant for quotes.'});
  SITES.push({url:'mobbed-funeral-parlor.vercel.app', live:'https://mobbed-funeral-parlor.vercel.app/', iframe:true, tag:'Memorial Services', name:'Mobbed Funeral Parlor', desc:'A dignified, calm digital presence for a funeral services provider &mdash; built for families to find information and reach the team without friction.'});

  function cardHTML(s){
    var preview = s.iframe ? '<iframe src="'+s.live+'" loading="lazy" style="width:250%;height:850px;border:0;transform:scale(0.4);transform-origin:top left;pointer-events:none;"></iframe>' : '<div style="position:static;height:100%;background:linear-gradient(160deg,#0c1420,#0a0f18);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;color:#90a4ae;font-size:13px;text-align:center;padding:20px;"><span style="font-size:1.8rem;">'+s.icon+'</span><span style="color:#fff;font-weight:600;font-size:1rem;">'+s.label+'</span><span>This site blocks in-page previews<br>for security &mdash; view it directly.</span><a href="'+s.live+'" target="_blank" rel="noopener" style="color:#FFB300;font-weight:600;font-size:.85rem;border:1px solid rgba(255,179,0,.4);padding:8px 18px;border-radius:6px;margin-top:4px;">Open Site &rarr;</a></div>';
    var html = '<div style="background:#0d1420;border:1px solid rgba(176,190,197,.12);border-radius:14px;overflow:hidden;">';
    html += '<div style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:#101826;border-bottom:1px solid rgba(176,190,197,.1);">';
    html += '<div style="display:flex;gap:6px;"><span style="width:9px;height:9px;border-radius:50%;background:#2a3441;display:inline-block;"></span><span style="width:9px;height:9px;border-radius:50%;background:#2a3441;display:inline-block;"></span><span style="width:9px;height:9px;border-radius:50%;background:#2a3441;display:inline-block;"></span></div>';
    html += '<div style="flex:1;font-family:monospace;font-size:11px;color:#90a4ae;background:rgba(0,0,0,.3);padding:5px 10px;border-radius:5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+s.url+'</div>';
    html += '<span style="width:7px;height:7px;border-radius:50%;background:#3DDC84;flex-shrink:0;"></span></div>';
    html += '<div style="position:relative;width:100%;height:280px;overflow:hidden;background:#000;">'+preview+'</div>';
    html += '<div style="padding:20px 22px 24px;">';
    html += '<div style="font-family:monospace;font-size:11px;color:#4FC3F7;text-transform:uppercase;letter-spacing:.08em;">'+s.tag+'</div>';
    html += '<h3 style="font-size:18px;margin-top:8px;font-weight:600;color:#fff;">'+s.name+'</h3>';
    html += '<p style="color:#90a4ae;font-size:13.5px;margin-top:8px;line-height:1.6;">'+s.desc+'</p>';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:18px;">';
    html += '<a href="'+s.live+'" target="_blank" rel="noopener" style="font-size:13px;font-weight:600;color:#FFB300;">Open Live Site &rarr;</a>';
    html += '<span style="font-size:11px;color:#90a4ae;">Live</span></div></div></div>';
    return html;
  }

  function buildPage(){
    var cards = '';
    for(var i=0;i<SITES.length;i++){ cards += cardHTML(SITES[i]); }

    var page = document.createElement('div');
    page.className = 'page';
    page.id = 'page-portfolio';

    var topbar = '<div style="position:sticky;top:0;z-index:40;display:flex;justify-content:space-between;align-items:center;padding:14px 6%;background:rgba(0,0,8,.9);backdrop-filter:blur(12px);border-bottom:1px solid rgba(176,190,197,.1);">';
    topbar += '<button onclick="gP(\'home\')" style="padding:8px 14px;border-radius:24px;background:rgba(79,195,247,.08);border:1px solid rgba(79,195,247,.25);color:#4FC3F7;font-weight:600;font-size:13px;cursor:pointer;">&larr; Back to Home</button>';
    topbar += '<a href="https://wa.me/27660018931" target="_blank" style="padding:8px 14px;border-radius:24px;background:#25d366;color:#fff;font-weight:700;font-size:13px;text-decoration:none;">WhatsApp</a></div>';

    var hero = '<section style="padding:60px 6% 50px;max-width:1100px;">';
    hero += '<div style="font-size:11px;color:#FFB300;letter-spacing:.12em;text-transform:uppercase;margin-bottom:22px;">6 live builds &middot; Vaal Triangle &amp; beyond</div>';
    hero += '<h1 style="font-family:Orbitron,sans-serif;font-size:clamp(28px,5.5vw,46px);font-weight:700;line-height:1.1;color:#fff;max-width:780px;">Websites that turn missed calls into booked customers.</h1>';
    hero += '<p style="margin-top:22px;font-size:16px;color:#90a4ae;max-width:580px;line-height:1.65;">A working portfolio of sites built and shipped for South African small businesses. Every card below is a real, live site.</p>';
    hero += '<div style="display:flex;gap:44px;margin-top:42px;flex-wrap:wrap;">';
    hero += '<div><strong style="display:block;font-size:30px;font-weight:700;color:#fff;">6</strong><span style="font-size:12px;color:#90a4ae;text-transform:uppercase;">Sites Shipped</span></div>';
    hero += '<div><strong style="display:block;font-size:30px;font-weight:700;color:#fff;">6</strong><span style="font-size:12px;color:#90a4ae;text-transform:uppercase;">Industries Served</span></div>';
    hero += '<div><strong style="display:block;font-size:30px;font-weight:700;color:#fff;">100%</strong><span style="font-size:12px;color:#90a4ae;text-transform:uppercase;">Mobile-Ready</span></div></div></section>';

    var work = '<section style="padding:30px 6% 60px;max-width:1300px;margin:0 auto;">';
    work += '<div style="margin-bottom:36px;max-width:600px;"><div style="font-size:11px;color:#4FC3F7;letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px;">The Work</div>';
    work += '<h2 style="font-size:clamp(22px,3.5vw,30px);font-weight:600;color:#fff;">Live sites, not mockups</h2>';
    work += '<p style="color:#90a4ae;margin-top:12px;font-size:14px;">Each preview below loads the actual deployed site.</p></div>';
    work += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:24px;">'+cards+'</div></section>';

    var cta = '<section style="padding:50px 6% 70px;text-align:center;">';
    cta += '<div style="background:rgba(79,195,247,.05);border:1px solid rgba(79,195,247,.15);border-radius:16px;padding:44px 28px;max-width:680px;margin:0 auto;">';
    cta += '<h3 style="font-size:20px;color:#fff;margin-bottom:12px;">Want your business looking like this?</h3>';
    cta += '<p style="color:#90a4ae;font-size:14px;margin-bottom:24px;">Basic sites delivered in 24-72 hours. Full systems in 14 days.</p>';
    cta += '<button onclick="gP(\'discovery\')" style="padding:13px 30px;background:#4FC3F7;color:#000008;font-weight:700;font-size:14px;border-radius:8px;border:none;cursor:pointer;">Get Started &rarr;</button></div></section>';

    var foot = '<footer style="text-align:center;padding:28px 6%;border-top:1px solid rgba(176,190,197,.1);font-size:12px;color:#78909c;">AutoSageAI &middot; Vaal Triangle, South Africa &middot; www.autosageai.co.za</footer>';

    page.innerHTML = topbar + hero + work + cta + foot;
  ...
  function addNavLinks(){
    var nlinks = document.querySelectorAll('.nlink');
    ...
    var mmls = document.querySelectorAll('.mml');
    for(var j=0;j<mmls.length;j++){
      var el2 = mmls[j];
      if(el2.textContent.replace(/^\s+|\s+$/g,'')==='Contact' && !document.getElementById('mml-portfolio')){
        var link2 = document.createElement('div');
        link2.className = 'mml';
        link2.id = 'mml-portfolio';
        link2.textContent = 'Portfolio';
        link2.onclick = function(){ gP('portfolio'); togMob(); };
        el2.parentNode.insertBefore(link2, el2);
      }
    }
    var fls = document.querySelectorAll('.ftlinks');
    for(var k=0;k<fls.length;k++){
      ...
      if(!fl.querySelector('.ftl-portfolio')){
        ...
        var ref = fl.firstChild ? fl.firstChild.nextSibling : null;
        fl.insertBefore(div, ref);
      }
    }
  }
